...
def generate_summary_tables(combined_df: pd.DataFrame, metrics_config: list) -> dict:
    """生成性能指标的汇总表格 - 使用analysis.tables模块"""
    try:
        from evaluation.analysis.tables import create_performance_summary_table
        
        # 分离不同方法的数据
        baseline_df = combined_df[combined_df["method_type"].str.contains("Baseline|基线", case=False, na=False)]
        h2o_df = combined_df[combined_df["method_type"].str.contains("H2O", case=False, na=False)]
        cake_df = combined_df[combined_df["method_type"].str.contains("CAKE", case=False, na=False)]
        
        # 使用专业的表格生成模块
        if len(h2o_df) > 0:
            summary_table = create_performance_summary_table(baseline_df, h2o_df)
        elif len(cake_df) > 0:
            summary_table = create_performance_summary_table(baseline_df, cake_df)
        else:
            summary_table = create_performance_summary_table(baseline_df, None)
        
        # 转换为字典格式
        return {"performance_summary": summary_table.to_dict(orient="records")}
        
    except ImportError:
        logger.warning("无法导入analysis.tables模块，使用简化版汇总表生成")
        # 保留原有的简化实现作为备选
        summary_tables = {}
