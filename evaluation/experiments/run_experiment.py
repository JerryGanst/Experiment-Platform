...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="示例用法:\n"
               "  python experiments/run_experiment.py baseline --datasets mmlu --kv_lengths 1024 --batch_sizes 1\n"
               "  python experiments/run_experiment.py h2o --datasets mmlu --h2o_ratios 0.1,0.2\n"
               "  python experiments/run_experiment.py cake --datasets mmlu --allocation_strategies adaptive --cache_budgets 0.7\n"
               "  python experiments/run_experiment.py compare_all --datasets mmlu,gsm8k --run_name full_comparison_run"
    )
...
def run_comparison_script(common_params:list, comparison_specific_params:list, run_output_dir:str, log_level:str, run_env:dict):
    ...
    # run_comparison.py位于当前experiments目录中
    comparison_script_path = os.path.join(os.path.dirname(__file__), "run_comparison.py")
    cmd = [sys.executable, comparison_script_path] + \
