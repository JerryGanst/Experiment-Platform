...
class UnifiedMonitor:
        ...
        if torch_available and torch.cuda.is_available():
            for device_idx in self.device_indices:
                try:
                    ...
                    logger.info(f"GPU {device_idx} 峰值内存统计已重置")
                except Exception as e:
        ...
        if torch_available and torch.cuda.is_available():
            for device_idx in self.device_indices:
                try:
                    ...
                    logger.info(f"GPU {device_idx} 最终内存状态: 当前分配={allocated:.1f}MB, 最终峰值={peak_allocated:.1f}MB")
                    
                    # 如果之前没有收集到峰值数据，确保至少记录最终状态
                    if len(self.gpu_stats[device_idx]["peak_memory_allocated"]) <= 1:
                        logger.warning(f"GPU {device_idx} 监控期间数据较少，强制记录最终峰值: {peak_allocated:.1f}MB")
                        
                except Exception as e:
                    logger.error(f"收集GPU {device_idx}最终统计信息失败: {e}")
    ...
    def _get_gpu_summary(self):
        ...
        for device_idx in self.device_indices:
            ...
            if device_stats.get("peak_memory_allocated") and device_stats["peak_memory_allocated"]:
                # 使用监控期间收集的数据
                peak_memory = max(device_stats["peak_memory_allocated"])
                avg_memory = np.mean(device_stats["memory_allocated"]) if device_stats["memory_allocated"] else 0
                sample_count = len(device_stats["timestamps"])
                
                logger.info(f"设备 {device_idx} 统计: 峰值={peak_memory:.1f}MB, 平均={avg_memory:.1f}MB, 样本={sample_count}")
            else:
                # 没有收集到数据
                peak_memory = 0
                avg_memory = 0
                sample_count = 0
                logger.warning(f"设备 {device_idx} 没有收集到GPU内存数据")
        ...
        logger.info(f"GPU统计摘要: 总峰值内存 {total_peak_memory:.1f}MB")
        return summary
