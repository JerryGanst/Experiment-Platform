...
h2o_experiment_dir = os.path.dirname(current_script_path) # Research Framework for Optimizing Head-level KV Cache Based on CAKE dir
project_root = os.path.dirname(h2o_experiment_dir) # project root
if project_root not in sys.path:
    sys.path.insert(0, project_root)

import config as global_config # 项目根目录的config
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="示例用法:\n"
               "  python Research Framework for Optimizing Head-level KV Cache Based on CAKE/run_experiment.py baseline --datasets mmlu --kv_lengths 1024 --batch_sizes 1\n"
               "  python Research Framework for Optimizing Head-level KV Cache Based on CAKE/run_experiment.py h2o --datasets mmlu --h2o_ratios 0.1,0.2\n"
               "  python Research Framework for Optimizing Head-level KV Cache Based on CAKE/run_experiment.py cake --datasets mmlu --allocation_strategies adaptive --cache_budgets 0.7\n"
               "  python Research Framework for Optimizing Head-level KV Cache Based on CAKE/run_experiment.py compare_all --datasets mmlu,gsm8k --run_name full_comparison_run"
    )
...
def run_single_experiment_script(script_name:str, common_params:list, specific_params:list, run_output_dir:str, log_level:str, run_env:dict):
    ...
    cmd = [sys.executable, os.path.join(h2o_experiment_dir, script_name)] + \
...
def run_comparison_script(common_params:list, comparison_specific_params:list, run_output_dir:str, log_level:str, run_env:dict):
    ...
    cmd = [sys.executable, os.path.join(h2o_experiment_dir, "run_comparison.py")] + \
...
def main():
    ...
    run_env["PYTHONPATH"] = project_root + os.pathsep + run_env.get("PYTHONPATH", "")
